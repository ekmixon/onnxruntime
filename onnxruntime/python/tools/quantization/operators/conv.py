...
class ConvInteger(QuantOperatorBase):
    ...
    def quantize(self):
        ...
        conv_integer_output = f"{node.output[0]}_output_quantized"
        conv_integer_name = f"{node.name}_quant" if node.name != "" else ""
        ...
        cast_op_output = f"{conv_integer_output}_cast_output"
        cast_node = onnx.helper.make_node(
            "Cast",
            [conv_integer_output],
            [cast_op_output],
            f"{conv_integer_output}_cast",
        ...
        if conv_integer_name != "":
            scales_mul_op = f"{conv_integer_name}_scales_mul"
        else:
            scales_mul_op = f"{scale_names[0]}_{scale_names[1]}_mul"
        ...
        output_scale_mul_op = (
            f"{conv_integer_name}_output_scale_mul"
            if conv_integer_name != ""
            else ""
        )
...
class QLinearConv(QuantOperatorBase):
    ...
    def quantize(self):
        ...
        qlinear_conv_output = f"{node.output[0]}_quantized"
        qlinear_conv_name = qlinear_conv_name = (
            f"{node.name}_quant" if node.name != "" else ""
        )
