...
class QLinearConcat(QuantOperatorBase):
    ...
    def quantize(self):
        ...
        if not data_found:
            raise ValueError(
                f'Quantization parameters for :\"{node.output[0]}\" of node:\"{node.name}\" not specified'
            )
        ...
        (
            ...
            scale_names,
            nodes,
        ) = self.quantizer.quantize_inputs(node, [*range(len(node.input))])
        ...
        qnode_name = f"{node.name}_quant" if node.name != "" else ""
